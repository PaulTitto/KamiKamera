...
<?php
	if (isset($_POST['simpanBarang'])) {
	    ...
	    $gambar=mysqli_query($conn, "SELECT img FROM tb_barang WHERE id_barang='$idBarang'");
	    $ambilGambar=mysqli_fetch_array($gambar);
		...
		if ($name==null) {
			echo "Kosong";
			$imageName=$ambilGambar['img'];
			$update=mysqli_query($conn, "UPDATE tb_barang SET `nama_barang`='$namaBarang', `harga_barang`='$hargaBarang', `harga_sewa`='$hargaSewa', `qty`='$qty', `img`='$imageName', `id_kategori`='$kategori' WHERE id_barang='$idBarang'") or die(mysqli_error($conn));
			if ($update) { ?>
