...
if (isset($_POST)) {	
    ...
    if ($daftar) {
        ...
        <script language="JavaScript">
			alertify.confirm('Data User berhasil Ditambah!', function(){window.location.href="daftar-user"}).setHeader(' ').set({closable:false,transition:'fade'}).autoOk(3); 
		</script>
		<?php
	}else{
		?>
		<script language="JavaScript">
			alertify.confirm('Data User gagal Ditambah!', function(){window.location.href="daftar-user"}).setHeader(' ').set({closable:false,transition:'fade'}).autoOk(3); 
		</script>
		<?php
    }
